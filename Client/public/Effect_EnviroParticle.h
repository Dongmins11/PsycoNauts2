...
BEGIN(Client)
class CEffect_EnviroParticle final :public CGameObject
{
public:
	enum UPDATE_TYPE { TYPE_DEFUALT, TYPE_ONEPOINT, TYPE_SPIRAL, TYPE_RANDOMPOS, TYPE_END };
private:
	explicit CEffect_EnviroParticle(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CEffect_EnviroParticle(const CEffect_EnviroParticle& rhs);
	virtual ~CEffect_EnviroParticle() = default;
...
public:
	static CEffect_EnviroParticle* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
