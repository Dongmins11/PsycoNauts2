...
class CHostZone final :public CGameObject
{
private:
	CHostZone(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CHostZone(const CHostZone& rhs);
	virtual ~CHostZone() = default;
...
private:/* Moveinfo */
	...
	/* For. Pass */
	_uint					m_iRenderPassArr[8] = { 0, 0, 0, 0, 0, 0, 0 };
	_bool					m_isEmissiveActive[8] = { false, false, true, true , true , true , true };
...
public:
	static CHostZone* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
