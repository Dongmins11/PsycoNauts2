...
BEGIN(Client)
class CSpinHole final :public CGameObject
{
...
protected:
	explicit CSpinHole(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CSpinHole(const CSpinHole& rhs);
	virtual ~CSpinHole() = default;
...
public:
	static CSpinHole* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
