...
class CStage4Mercial final :public CGameObject
{
private:
	CStage4Mercial(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CStage4Mercial(const CStage4Mercial& rhs);
	virtual ~CStage4Mercial() = default;
...
private:/* Moveinfo */
	...
	/* For. Pass */
	_uint					m_iRenderPassArr[5] = { 0, 0, 0, 0, 0 };
	_bool					m_isEmissiveActive[5] = { false, false, false, false, true };

	/*For. Reflect*/
	_float					m_fReflect_Height = 0.f;
public:
	static CStage4Mercial* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
