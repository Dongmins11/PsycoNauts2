#include "stdafx.h"
#include "..\public\PlatingPlate_Phase2.h"
#include "GameInstance.h"

CPlatingPlate_Phase2::CPlatingPlate_Phase2(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CGameObject(pDevice,pDeviceContext)
{
}

CPlatingPlate_Phase2::CPlatingPlate_Phase2(const CPlatingPlate_Phase2 & rhs)
	:CGameObject(rhs)
	,m_vecNaviData(rhs.m_vecNaviData)
{
}

HRESULT CPlatingPlate_Phase2::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype()))
		return E_FAIL;

	Loading_PlateNaviData(TEXT("../../Data/Stage03/NaviPlating2.dat"));
	return S_OK;
}

HRESULT CPlatingPlate_Phase2::NativeConstruct(void * pArg, _uint iCurrentScene)
{
	...
	m_StrLayerName = TEXT("Layer_Plating_Plate2");//objinfo.StrLayerName;
	m_StrModelName = TEXT("Prototype_Component_Model_Compt_Plating_Plate2");//objinfo.StrModelName;
	m_StrObjectFullName = TEXT("Prototype_GameObject_Plating_Plate2"); //objinfo.StrObjectFullName;
...
_int CPlatingPlate_Phase2::Tick(_double TimeDelta)
{
...
_int CPlatingPlate_Phase2::LateTick(_double TimeDelta)
{
...
HRESULT CPlatingPlate_Phase2::Render()
{
...
_bool CPlatingPlate_Phase2::Checking_FruitCooking(FruitType eFruitType, CookType eCookType, _uint* iCookingUINumber)
{
	_int iCheckFruit = -1;
	...
	if (((_uint)m_vecCookTypes[iCheckFruit] & (2 << (_uint)eCookType)) == (2 << (_uint)eCookType))
	{
		if (eCookType != CookType::COOK_CUT && (_uint)m_vecCookTypes[iCheckFruit] >= 24)
		{
			...
			{
				if(iCheckFruit != 0)
					*iCookingUINumber = m_vecPlateFruits.size() + 1;
			}
		}
		return true;
	}
...
HRESULT CPlatingPlate_Phase2::SetUp_Component()
{
...
HRESULT CPlatingPlate_Phase2::SetUp_ConstantTable()
{
...
HRESULT CPlatingPlate_Phase2::Loading_PlateNaviData(const _tchar * pNaviDataFilePath)
{
	HANDLE			hFile = 0;
	_uint			iCntPoint = 3;
	_ulong	dwByte = 0;
...
HRESULT CPlatingPlate_Phase2::SetUp_Triggers()
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
	_matrix mat = XMMatrixScaling(4.f, 3.f, 4.f) * XMMatrixIdentity();//XMMatrixTranslation(-4.f, 1.f, 0.f);
	_vector LookVec = mat.r[2];
	LookVec = XMVectorSetW(LookVec, (2 << (_uint)CookType::COOK_BAKE));
	mat.r[2] = LookVec;
	mat.r[3] = XMVectorSet(233.5f, 2.55f, 195.5f, (_float)FruitType::FRUIT_EGG);
	...
	m_vecCookTypes.emplace_back((2 << (_uint)CookType::COOK_BAKE));
	...
	mat.r[2] = LookVec;
	mat.r[3] = XMVectorSet(240.6f, 2.55f, 195.8f, (_float)FruitType::FRUIT_PORK);
	pGameInstance->Add_ObjectToLayer_GetImmediately(SCENE_STAGE03, TEXT("Prototype_GameObject_FruitTrigger"), SCENE_STAGE03, TEXT("Layer_PlateTrigger"), m_pFruitCollider, &mat);

	m_vecPlateFruits.emplace_back(FruitType::FRUIT_PORK);
	...
	mat = XMMatrixScaling(4.f, 3.f, 4.f) * XMMatrixIdentity();//XMMatrixTranslation(-4.f, 1.f, 0.f);
	LookVec = mat.r[2];
	LookVec = XMVectorSetW(LookVec, (2 << (_uint)CookType::COOK_BOIL));
	mat.r[2] = LookVec;
	mat.r[3] = XMVectorSet(240.4f, 2.55f, 188.3f, (_float)FruitType::FRUIT_BEEHIVE);
	pGameInstance->Add_ObjectToLayer_GetImmediately(SCENE_STAGE03, TEXT("Prototype_GameObject_FruitTrigger"), SCENE_STAGE03, TEXT("Layer_PlateTrigger"), m_pFruitCollider, &mat);

	m_vecPlateFruits.emplace_back(FruitType::FRUIT_BEEHIVE);
	m_vecCookTypes.emplace_back((2 << (_uint)CookType::COOK_BOIL));


	mat = XMMatrixScaling(4.f, 3.f, 4.f) * XMMatrixIdentity();//XMMatrixTranslation(-4.f, 1.f, 0.f);
	LookVec = mat.r[2];
	LookVec = XMVectorSetW(LookVec, (2 << (_uint)CookType::COOK_CUT) + (2 << (_uint)CookType::COOK_BLEND));
	mat.r[2] = LookVec;
	mat.r[3] = XMVectorSet(233.f, 2.55f, 188.3f, (_float)FruitType::FRUIT_WATERMELON);
	pGameInstance->Add_ObjectToLayer_GetImmediately(SCENE_STAGE03, TEXT("Prototype_GameObject_FruitTrigger"), SCENE_STAGE03, TEXT("Layer_PlateTrigger"), m_pFruitCollider, &mat);

	m_vecPlateFruits.emplace_back(FruitType::FRUIT_WATERMELON);
	m_vecCookTypes.emplace_back((2 << (_uint)CookType::COOK_CUT) + (2 << (_uint)CookType::COOK_BLEND));
...
#ifdef _DEBUG
HRESULT CPlatingPlate_Phase2::SetUp_RenderComponentForDebug()
{
...
CPlatingPlate_Phase2 * CPlatingPlate_Phase2::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CPlatingPlate_Phase2*	pInstance = new CPlatingPlate_Phase2(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created : CPlatingPlate_Phase2");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CPlatingPlate_Phase2::Clone(void * pArg, _uint iCurrentScene)
{
	CPlatingPlate_Phase2*	pInstance = new CPlatingPlate_Phase2(*this);
...
void CPlatingPlate_Phase2::Free()
{
	__super::Free();
