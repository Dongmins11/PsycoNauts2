#include "stdafx.h"
#include "..\public\GoatHollis.h"
#include "GameInstance.h"
#include "CollisionObject.h"
#include "Sequence.h"
#include "Dead_GoatH.h"
#include "Idle_GoatH.h"
#include "Hit_GoatH.h"
#include "Attack_GoatH.h"
#include "Taunt_GoatH.h"
#include "Spawn_GoatH.h"
#include "Boss_Goat.h"
#include "AttTrigger_GoatH.h"

CGoatHollis::CGoatHollis(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CEnemy(pDevice, pDeviceContext)
{
}

CGoatHollis::CGoatHollis(const CGoatHollis & rhs)
	: CEnemy(rhs)
{
}

HRESULT CGoatHollis::NativeConstruct_Prototype()
{
...
HRESULT CGoatHollis::NativeConstruct(void * pArg, _uint iCurrentScene)
{
	...
	m_pAttackGoat = CAttack_GoatH::Create(this);
	m_pDeadGoat = CDead_GoatH::Create(this);
	m_pHitGoat = CHit_GoatH::Create(this);
	m_pIdleGoat = CIdle_GoatH::Create(this);
	m_pTauntGoat = CTaunt_GoatH::Create(this);
	m_pSpawnGoat = CSpawn_GoatH::Create(this);
	...
	m_pAttTrigger = CAttTrigger_GoatH::Create(m_pDevice, m_pDeviceContext, &m_BoneMatrix);

	return S_OK;
}

_int CGoatHollis::Tick(_double TimeDelta)
{
...
_int CGoatHollis::LateTick(_double TimeDelta)
{
...
void CGoatHollis::Reset()
{
	m_pHitGoat->ResetHit();
	m_pAttackGoat->Reset_AttackState();
}

void CGoatHollis::OnTriggerEnter(CCollisionObject & collision)
{
...
void CGoatHollis::OnCollisionEnter(CCollisionObject & collision)
{
}

void CGoatHollis::OnControllerCollisionHit(CCollisionObject & collision)
{
}

void CGoatHollis::OnTriggerExit(CCollisionObject & collision)
{
}

void CGoatHollis::Update_LookVector(_double TimeDelta)
{
...
void CGoatHollis::LockBosses(_bool * _bArr)
{
	static_cast<CBoss_Goat*>(m_pParent)->Lock_Boss(_bArr);
}

HRESULT CGoatHollis::SetUp_Component()
{
	if (FAILED(__super::SetUp_Component(SCENE_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
		return E_FAIL;

	/* Com_Model*/
	if (FAILED(__super::SetUp_Component(SCENE_STATIC, TEXT("Prototype_Component_Model_GoatHollis0"), TEXT("Com_VIBuffer"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CGoatHollis::SetUp_ConstantTable()
{
...
CGoatHollis * CGoatHollis::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext,void* pArg)
{
	CGoatHollis*	pInstance = new CGoatHollis(pDevice, pDeviceContext);
...
CGameObject * CGoatHollis::Clone(void * pArg, _uint iCurrentScene)
{
	return nullptr;
}

void CGoatHollis::Free()
{
	__super::Free();
