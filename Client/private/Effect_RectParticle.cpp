#include "stdafx.h"
#include "..\Public\Effect_RectParticle.h"
#include "GameInstance.h"

CEffect_RectParticle::CEffect_RectParticle(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
...
HRESULT CEffect_RectParticle::NativeConstruct_Prototype()
{
...
HRESULT CEffect_RectParticle::NativeConstruct(void * pArg, _uint iCurrentScene)
{
...
_int CEffect_RectParticle::Tick(_double TimeDelta)
{
...
_int CEffect_RectParticle::LateTick(_double TimeDelta)
{
	...
	case TYPE_FIRE:
		...
		break;
	case TYPE_PARTICLE:
		m_pVIBufferCom->Update_Particle(&m_tParticleInfo, m_pOptionCom->Get_ParentTransform()->Get_Position(), pGameInstance->Get_CamPosition(), TimeDelta);
		break;
	...
	case TYPE_WATER:
		...
		break;
	case TYPE_VENTSMOKE:
		m_pVIBufferCom->Update_VentSmoke(&m_tParticleInfo, m_pOptionCom->Get_WorldPosMatrix().r[3], pGameInstance->Get_CamPosition(),  TimeDelta);
		break;
	...
	if (TYPE_SMOKE_WORK != m_tParticleInfo.iOption ||
		TYPE_PARTICLE != m_tParticleInfo.iOption)
	...
	{
		m_pRendererCom->Add_RenderGroup(CRenderer::RENDER_MODEL_EFFECT_AFTER, this);
	}
	else
	{
...
HRESULT CEffect_RectParticle::SetUp_Component(wstring	wstrVIBufferTag)
{
	/* Com_Renderer */
...
HRESULT CEffect_RectParticle::SetUp_ConstantTable()
{
	...
	if (TYPE_PAPER == m_tParticleInfo.iOption)
	{
		...
		if (FAILED(m_pVIBufferCom->Set_RawValue("g_bEmissive", &bEmissive, sizeof(_bool))))
			return E_FAIL;
	}
	RELEASE_INSTANCE(CGameInstance);
...
CEffect_RectParticle * CEffect_RectParticle::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CEffect_RectParticle* pInstance = new CEffect_RectParticle(pDevice, pDeviceContext);
...
CGameObject * CEffect_RectParticle::Clone(void * pArg, _uint iCurrentScene)
{
