#include "stdafx.h"
#include "..\public\Bread.h"
#include "GameInstance.h"

CBread::CBread(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
...
HRESULT CBread::NativeConstruct_Prototype()
{
...
HRESULT CBread::NativeConstruct(void * pArg, _uint iCurrentScene)
{
	...
	m_StrLayerName = TEXT("Layer_BREAD");
	Setting_ModelNames();
	m_StrObjectFullName = TEXT("Prototpye_GameObject_BREAD");
	...
	//테스트코드
	//m_iCookingNumber = (2 << (_uint)CookType::COOK_CUT) + (2 << (_uint)CookType::COOK_BAKE);
	return S_OK;
}

_int CBread::Tick(_double TimeDelta)
{
...
_int CBread::LateTick(_double TimeDelta)
{
...
void CBread::Setting_ModelNames()
{
	m_StrModelName[FMT_IDLE] = TEXT("Prototype_Component_Model_Bread0");
	m_StrModelName[FMT_BOILED] = TEXT("Prototype_Component_Model_Bread0");
	m_StrModelName[FMT_CUTTING] = TEXT("Prototype_Component_Model_BreadSliced0");
	m_StrModelName[FMT_BLENDED] = TEXT("Prototype_Component_Model_Bread0");
	m_StrModelName[FMT_GRIDDLED] = TEXT("Prototype_Component_Model_BreadGriddled0");
}

HRESULT CBread::SetUp_Component()
{
	/* Com_Renderer */
...
HRESULT CBread::SetUp_ConstantTable()
{
...
CBread * CBread::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CBread*	pInstance = new CBread(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created : CBread");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CBread::Clone(void * pArg, _uint iCurrentScene)
{
	CBread*	pInstance = new CBread(*this);
