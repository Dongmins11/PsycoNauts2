#include "stdafx.h"
#include "..\public\AWaterMelon.h"

CAWaterMelon::CAWaterMelon(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CFruit(pDevice, pDeviceContext)
{
}

CAWaterMelon::CAWaterMelon(const CAWaterMelon & rhs)
	: CFruit(rhs)
{
}

HRESULT CAWaterMelon::NativeConstruct_Prototype()
{
...
HRESULT CAWaterMelon::NativeConstruct(void * pArg, _uint iCurrentScene)
{
	...
	m_StrLayerName = TEXT("Layer_AWATERMELON");
	Setting_ModelNames();
	m_StrObjectFullName = TEXT("Prototpye_GameObject_AWATERMELON");
	...
	////테스트코드
	//m_iCookingNumber = (2 << (_uint)CookType::COOK_BLEND) + (2 << (_uint)CookType::COOK_CUT);
	return S_OK;
}

_int CAWaterMelon::Tick(_double TimeDelta)
{
...
_int CAWaterMelon::LateTick(_double TimeDelta)
{
...
void CAWaterMelon::Setting_ModelNames()
{
	m_StrModelName[FMT_IDLE] = TEXT("Prototype_Component_Model_AWatermelon0");
	m_StrModelName[FMT_BOILED] = TEXT("Prototype_Component_Model_AWatermelon0");
	m_StrModelName[FMT_CUTTING] = TEXT("Prototype_Component_Model_AWatermelonSliced0");
	m_StrModelName[FMT_BLENDED] = TEXT("Prototype_Component_Model_AWatermelonBlended0");
	m_StrModelName[FMT_GRIDDLED] = TEXT("Prototype_Component_Model_AWatermelon0");
}

HRESULT CAWaterMelon::SetUp_Component()
{
	/* Com_Renderer */
...
HRESULT CAWaterMelon::SetUp_ConstantTable()
{
...
HRESULT CAWaterMelon::Create_Effect()
{
	if (!m_isInteraction)
	{
...
HRESULT CAWaterMelon::Change_Effect()
{
...
CAWaterMelon * CAWaterMelon::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CAWaterMelon*	pInstance = new CAWaterMelon(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created : CAWaterMelon");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CAWaterMelon::Clone(void * pArg, _uint iCurrentScene)
{
	CAWaterMelon*	pInstance = new CAWaterMelon(*this);
