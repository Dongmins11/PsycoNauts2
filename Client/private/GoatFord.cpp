#include "stdafx.h"
#include "..\public\GoatFord.h"
#include "GameInstance.h"
#include "CollisionObject.h"
#include "Sequence.h"
#include "Taunt_GoatF.h"
#include "Idle_GoatF.h"
#include "Dead_GoatF.h"
#include "Attack_GoatF.h"
#include "Hit_GoatF.h"
#include "Spawn_GoatF.h"
#include "Boss_Goat.h"
#include "AttTrigger_GoatF.h"
#include "Scene_Loading.h"

CGoatFord::CGoatFord(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CEnemy(pDevice, pDeviceContext)
{
}

CGoatFord::CGoatFord(const CGoatFord & rhs)
	: CEnemy(rhs)
{
}

HRESULT CGoatFord::NativeConstruct_Prototype()
{
...
HRESULT CGoatFord::NativeConstruct(void * pArg, _uint iCurrentScene)
{
	...
	m_pAttackGoat = CAttack_GoatF::Create(this);
	m_pDeadGoat = CDead_GoatF::Create(this);
	m_pHitGoat = CHit_GoatF::Create(this);
	m_pIdleGoat = CIdle_GoatF::Create(this);
	m_pTauntGoat = CTaunt_GoatF::Create(this);
	m_pSpawnGoat = CSpawn_GoatF::Create(this);
	...
	m_pModelCom->Set_AnimBlendSpeed(18, 1.0);
	m_pAttTrigger = CAttTrigger_GoatF::Create(m_pDevice, m_pDeviceContext, &m_BoneMatrix);

	return S_OK;
}

_int CGoatFord::Tick(_double TimeDelta)
{
	...
	if (true == m_pRoot->Invoke(TimeDelta, m_pModelCom, m_pTransformCom, nullptr))
	{
		...
		if (FAILED(pGameInstance->Open_Scene(SCENE_LOADING, CScene_Loading::Create(m_pDevice, m_pDeviceContext, SCENE_ENDING))))
		{
...
_int CGoatFord::LateTick(_double TimeDelta)
{
...
void CGoatFord::OnTriggerEnter(CCollisionObject & collision)
{
	if (collision.Get_ObjectLayerType() == LayerType::LAYERTYPE_PLAYER_SKILL)
	{
		...
		CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
		((CBoss_Goat*)pGameInstance->Get_GameObject(SCENE_STAGE05, TEXT("Layer_FinalBoss"), 0))->Damaged(3);
...
void CGoatFord::OnCollisionEnter(CCollisionObject & collision)
{
}

void CGoatFord::OnControllerCollisionHit(CCollisionObject & collision)
{
}

void CGoatFord::OnTriggerExit(CCollisionObject & collision)
{
}

void CGoatFord::Reset()
{
	m_pHitGoat->ResetHit();
	m_pAttackGoat->Reset_AttackState();
}

void CGoatFord::Update_LookVector(_double TimeDelta)
{
...
void CGoatFord::LockBosses(_bool * _bArr)
{
	static_cast<CBoss_Goat*>(m_pParent)->Lock_Boss(_bArr);
}

void CGoatFord::Set_LinkSkill(_bool _bLinkSkill)
{
...
_bool CGoatFord::IsLinkSkill()
{
	return m_IsLinkSkill;
}

HRESULT CGoatFord::SetUp_Component()
{
	...
	tBoxColliderDesc.vScale = { 7.f,10.f,7.f };//MULTI_FLOAT3(vColSize, 2.0);
...
HRESULT CGoatFord::SetUp_ConstantTable()
{
...
CGoatFord * CGoatFord::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, void* pArg)
{
	CGoatFord*	pInstance = new CGoatFord(pDevice, pDeviceContext);
...
CGameObject * CGoatFord::Clone(void * pArg, _uint iCurrentScene)
{
	return nullptr;
}
