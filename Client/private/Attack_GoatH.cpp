#include "stdafx.h"
#include "..\public\Attack_GoatH.h"
#include "Boss_Goat.h"
#include "Puke_Ball.h"

CAttack_GoatH::CAttack_GoatH(CGoatHollis * _pGoat)
	:m_pGoat(_pGoat)
{
}

_bool CAttack_GoatH::Invoke(_double TimeDelta, CModel * _pModel, CTransform * _pTransform, CCharacterController* _pController)
{
	if (0 >= m_pGoat->Get_Hp())
		return true;
	...
	else
	{
		if (m_IsAttackLoop)
		{
			...
			else if (2 == m_iAttackType)
			{
				if (!m_IsReadyAttack)
				{
					_bool bArr[CBoss_Goat::BOSS::BOSS_END] = { false, true , true, true, true };
					m_pGoat->LockBosses(bArr);
					_pModel->Set_IsAnimLefp(true);
		...
		else if (4 == _pModel->Get_AnimIndex())
		{
				...
				32 <= _pModel->Get_AnimationCurrentKeyFrame())
			{
				...
				if (FAILED(pGameInstance->Add_ObjectToEffect(SCENE_STATIC, TEXT("Prototype_GameObject_Effect_RectParticle"),
					...
					SCENE_STATIC, TEXT("Effect_Boss_SmokeEffectHollis"))))
				{
					RELEASE_INSTANCE(CGameInstance);
					return E_FAIL;
				}
...
void CAttack_GoatH::Reset_AttackState()
{
	m_dTelegraphLoopTime = 0.0;
	m_dStrikeLoopTime = 0.0;
	m_IsDelayAttack = false;
...
CAttack_GoatH * CAttack_GoatH::Create(CGoatHollis * _pGoat)
{
	CAttack_GoatH* pInstance = new CAttack_GoatH(_pGoat);

	return pInstance;
}

void CAttack_GoatH::Free()
{
}
