...
CAWaterCooler::CAWaterCooler(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CGameObject(pDevice, pDeviceContext)
{
}

CAWaterCooler::CAWaterCooler(const CAWaterCooler & rhs)
	: CGameObject(rhs)
{
}

HRESULT CAWaterCooler::NativeConstruct_Prototype()
{
...
HRESULT CAWaterCooler::NativeConstruct(void * pArg)
{
	...
	if (FAILED(pGameInstance->Add_ObjectToParticle_GetImmediately(SCENE_STATIC, TEXT("Prototype_GameObject_Effect_RectRepeat"),
		m_iCurrentScene, TEXT("Layer_Env_WaterObject"),
		SCENE_STATIC, TEXT("Effect_Env_WaterObject"), &m_pEffect)))
	{
	...
	if (FAILED(pGameInstance->Effect_Call_Inheritance(m_iCurrentScene, false, false, m_pTransformCom, TEXT("Layer_Env_WaterObject"))))
	{
		RELEASE_INSTANCE(CGameInstance);
		return -1;
	}
	pGameInstance->Setting_Effect_TransformPos(m_iCurlingIndex, TEXT("Layer_Env_WaterObject"), TEXT("Com_Option"), CTransform::STATE_UP);
...
_int CAWaterCooler::Tick(_double TimeDelta)
{
...
_int CAWaterCooler::LateTick(_double TimeDelta)
{
...
void CAWaterCooler::Fluctuate(_double TimeDelta)
{
...
void CAWaterCooler::Add_Force(_fvector vDir, PxForceMode::Enum eMode)
{
	m_pColliderCom->Add_Force(vDir, eMode);

}

void CAWaterCooler::Setting_Throwing(_fvector vDir)
{
...
void CAWaterCooler::OnCollisionEnter(CCollisionObject & collision)
{
	if (!m_isMove)
		return;
...
void CAWaterCooler::OnCollisionStay(CCollisionObject & collision)
{
}

void CAWaterCooler::OnCollisionExit(CCollisionObject & collision)
{
}

void CAWaterCooler::OnTriggerEnter(CCollisionObject & collision)
{
...
void CAWaterCooler::SettingActorActive()
{
	if (!m_pColliderCom)
		return;
...
_bool CAWaterCooler::Setting_CurlingIndex(_int * pCurlingIndex, _int * pDeleteIndex)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
void CAWaterCooler::Stage_RenderBranch(_int iPlayerCurlingIndex)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CAWaterCooler::SetUp_Component(_float3 vColSize, _float3 vColPos, _float3 vObjScale)
{
	m_vColPos = vColPos;
	/* Com_Renderer */
...
HRESULT CAWaterCooler::SetUp_ConstantTable()
{
...
void CAWaterCooler::OnControllerCollisionHit(CCollisionObject & collision)
{
	if (m_eLayerType == LayerType::LAYERTYPE_PICK)
	{
		m_isMove = true;
...
HRESULT CAWaterCooler::Create_Effect()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);

	if (FAILED(pGameInstance->Add_ObjectToEffect(SCENE_STATIC, TEXT("Prototype_GameObject_Effect_RectParticle"),
		pGameInstance->Get_CurrentSceneIndex(), TEXT("Layer_WaterRect"),
		SCENE_STATIC, TEXT("Effect_WaterRect"))))
	{
		RELEASE_INSTANCE(CGameInstance);
		return false;
	}
	...
	if (FAILED(pGameInstance->Effect_Call_Inheritance(pGameInstance->Get_CurrentSceneIndex(), true, false, m_pTransformCom, TEXT("Layer_WaterRect"))))
	{
		RELEASE_INSTANCE(CGameInstance);
		return false;
	}

	pGameInstance->Setting_Effect_NonRotation(pGameInstance->Get_CurrentSceneIndex(), TEXT("Layer_WaterRect"), TEXT("Com_Option"), true);
...
CAWaterCooler * CAWaterCooler::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CAWaterCooler*	pInstance = new CAWaterCooler(pDevice, pDeviceContext);
...
CGameObject * CAWaterCooler::Clone(void * pArg, _uint iSceneIndex)
{
	CAWaterCooler*	pInstance = new CAWaterCooler(*this);
