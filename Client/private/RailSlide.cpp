#include "stdafx.h"
#include "..\public\RailSlide.h"
#include "GameInstance.h"
#include "Camera_Fly.h"
#include "Player.h"
#include "CollisionObject.h"
CRailSlide::CRailSlide(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CGameObject(pDevice, pDeviceContext)
{
}

CRailSlide::CRailSlide(const CRailSlide & rhs)
	: CGameObject(rhs)
{
}

HRESULT CRailSlide::NativeConstruct_Prototype()
{
...
HRESULT CRailSlide::NativeConstruct(void * pArg, _uint iCurrentScene)
{
	...
	m_eLayerType = LayerType::LAYERTYPE_RAIL;
	...
	if (nullptr != m_pColliderCom)
	{
		...
		m_pColliderCom->Update_Transform(&m_pTransformCom->Get_WorldFloat4x4());
	}
...
_int CRailSlide::Tick(_double TimeDelta)
{
...
_int CRailSlide::LateTick(_double TimeDelta)
{
...
void CRailSlide::OnCollisionEnter(CCollisionObject & collision)
{
	if (collision.Get_ObjectLayerType() == LayerType::LAYERTYPE_PLAYER_TRIGGER|| collision.Get_ObjectLayerType() == LayerType::LAYERTYPE_PLAYER)
	{
		//CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
		//CPlayer* Player = (CPlayer*)pGameInstance->Get_GameObject(SCENE_STATIC, TEXT("Layer_Player"), 0);
		//Player->Set_RailSlide(true, m_pTransformCom->Get_State(CTransform::STATE_UP));
		//RELEASE_INSTANCE(CGameInstance);
		//OutputDebugString(TEXT("Ãæµ¹"));
	}
}

_float3 CRailSlide::Get_Up()
{
	return m_pTransformCom->Get_State_Float3(CTransform::STATE_UP);
}

_vector CRailSlide::Get_Pos()
{
	return m_pTransformCom->Get_Position();
}

_bool CRailSlide::Setting_CurlingIndex(_int * pCurlingIndex, _int * pDeleteIndex)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
	CPlayer* Player = (CPlayer*)pGameInstance->Get_GameObject(SCENE_STATIC, TEXT("Layer_Player"), 0);
	if (Player)
...
void CRailSlide::Stage_RenderBranch(_int iPlayerCurlingIndex)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CRailSlide::SetUp_Component(_float3 vColSize, _float3 vColPos, _float3 vObjScale)
{
	m_vColPos = vColPos;
	/* Com_Renderer */
...
HRESULT CRailSlide::SetUp_ConstantTable()
{
...
CRailSlide * CRailSlide::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CRailSlide*	pInstance = new CRailSlide(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created : CRailSlide");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CRailSlide::Clone(void * pArg, _uint iCurrentScene)
{
	CRailSlide*	pInstance = new CRailSlide(*this);
