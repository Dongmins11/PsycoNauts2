...
CBeeHive::CBeeHive(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CFruit(pDevice, pDeviceContext)
{
}

CBeeHive::CBeeHive(const CBeeHive & rhs)
	: CFruit(rhs)
{
}

HRESULT CBeeHive::NativeConstruct_Prototype()
{
...
HRESULT CBeeHive::NativeConstruct(void * pArg, _uint iCurrentScene)
{
	...
	m_StrLayerName = TEXT("Layer_BEEHIVE");
	Setting_ModelNames();
	m_StrObjectFullName = TEXT("Prototpye_GameObject_BEEHIVE");
	...
	//테스트코드
	//m_iCookingNumber = (2 << (_uint)CookType::COOK_BOIL);
	return S_OK;
}

_int CBeeHive::Tick(_double TimeDelta)
{
...
_int CBeeHive::LateTick(_double TimeDelta)
{
...
void CBeeHive::Setting_ModelNames()
{
	m_StrModelName[FMT_IDLE] = TEXT("Prototype_Component_Model_BeeHive0");
	m_StrModelName[FMT_BOILED] = TEXT("Prototype_Component_Model_BeeHiveBoiled0");
	m_StrModelName[FMT_CUTTING] = TEXT("Prototype_Component_Model_BeeHive0");
	m_StrModelName[FMT_BLENDED] = TEXT("Prototype_Component_Model_BeeHive0");
	m_StrModelName[FMT_GRIDDLED] = TEXT("Prototype_Component_Model_BeeHive0");
}

HRESULT CBeeHive::SetUp_Component()
{
	/* Com_Renderer */
...
HRESULT CBeeHive::SetUp_ConstantTable()
{
...
HRESULT CBeeHive::Create_Effect()
{
	if (!m_isInteraction)
	{
...
HRESULT CBeeHive::Change_Effect()
{
...
CBeeHive * CBeeHive::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CBeeHive*	pInstance = new CBeeHive(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created : CBeeHive");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CBeeHive::Clone(void * pArg, _uint iCurrentScene)
{
	CBeeHive*	pInstance = new CBeeHive(*this);
