#include "stdafx.h"
#include "..\public\StationRamp1.h"
#include "GameInstance.h"
#include "Camera_Fly.h"
#include "Player.h"

CStationRamp1::CStationRamp1(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CGameObject(pDevice, pDeviceContext)
{
}

CStationRamp1::CStationRamp1(const CStationRamp1 & rhs)
	: CGameObject(rhs)
{
}

HRESULT CStationRamp1::NativeConstruct_Prototype()
{
...
HRESULT CStationRamp1::NativeConstruct(void * pArg, _uint iCurrentScene)
{
	...
	m_StrLayerName = TEXT("Layer_Compt_StationRamp1");
...
_int CStationRamp1::Tick(_double TimeDelta)
{
...
_int CStationRamp1::LateTick(_double TimeDelta)
{
...
void CStationRamp1::OnCollisionEnter(CCollisionObject & collision)
{
}

_bool CStationRamp1::Setting_CurlingIndex(_int * pCurlingIndex, _int * pDeleteIndex)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
void CStationRamp1::Stage_RenderBranch(_int iPlayerCurlingIndex)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CStationRamp1::SetUp_Component(_float3 vColSize, _float3 vColPos, _float3 vObjScale)
{
	m_vColPos = vColPos;
	/* Com_Renderer */
...
HRESULT CStationRamp1::SetUp_ConstantTable()
{
	...
	m_pModelCom->Set_TextureValue("g_LightImgTexture", m_pLightMapCom->Get_SRV(1));
...
CStationRamp1 * CStationRamp1::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CStationRamp1*	pInstance = new CStationRamp1(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created : CStationRamp1");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CStationRamp1::Clone(void * pArg, _uint iCurrentScene)
{
	CStationRamp1*	pInstance = new CStationRamp1(*this);
