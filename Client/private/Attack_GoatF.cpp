#include "stdafx.h"
#include "..\public\Attack_GoatF.h"
#include "Boss_Goat.h"
#include "Puke_Ball.h"

CAttack_GoatF::CAttack_GoatF(CGoatFord * _pGoat)
	:m_pGoat(_pGoat)
{
}

_bool CAttack_GoatF::Invoke(_double TimeDelta, CModel * _pModel, CTransform * _pTransform, CCharacterController* _pController)
{
	if (0 >= m_pGoat->Get_Hp())
		return true;
	...
	if (_pModel->Get_IsAnimFinishied(1) ||
		...
		_pModel->Get_IsAnimFinishied(21))
	{
		...
		if (1 == _pModel->Get_AnimIndex())
		{
			...
			m_pGoat->Set_LinkSkill(false);
		}
		else if (21 == _pModel->Get_AnimIndex()) {
	...
	else
	{
		if (m_IsAttackLoop)
		{
			...
			else if (2 == m_iAttackType)
			{
				if (!m_IsReadyAttack)
				{
					_bool bArr[CBoss_Goat::BOSS::BOSS_END] = { true, true , false, true, true };
					m_pGoat->LockBosses(bArr);
					_pModel->Set_IsAnimLefp(true);
		...
		else if (4 == _pModel->Get_AnimIndex())
		{
				...
				32 <= _pModel->Get_AnimationCurrentKeyFrame())
			{
				...
				if (FAILED(pGameInstance->Add_ObjectToEffect(SCENE_STATIC, TEXT("Prototype_GameObject_Effect_RectParticle"),
					...
					SCENE_STATIC, TEXT("Effect_Boss_SmokeEffectFord"))))
				{
					RELEASE_INSTANCE(CGameInstance);
					return E_FAIL;
				}
		...
		{
			if (!m_IsShoot[0] &&
				29 <= _pModel->Get_AnimationCurrentKeyFrame())
			{
				CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
				CPuke_Ball::BallWorldDesc tDesc;
				tDesc.fDistance = m_pGoat->Get_Distance();
...
void CAttack_GoatF::Reset_AttackState()
{
	m_dDelayTime = 0.0;
	m_dPukeDelayTime = 0.0;
	m_dTelegraphLoopTime = 0.0;
...
CAttack_GoatF * CAttack_GoatF::Create(CGoatFord * _pGoat)
{
	CAttack_GoatF* pInstance = new CAttack_GoatF(_pGoat);

	return pInstance;
}

void CAttack_GoatF::Free()
{
}
