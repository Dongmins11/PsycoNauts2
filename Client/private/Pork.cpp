#include "stdafx.h"
#include "..\public\Pork.h"

CPork::CPork(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CFruit(pDevice, pDeviceContext)
{
}

CPork::CPork(const CPork & rhs)
	: CFruit(rhs)
{
}

HRESULT CPork::NativeConstruct_Prototype()
{
...
HRESULT CPork::NativeConstruct(void * pArg, _uint iCurrentScene)
{
	...
	m_StrLayerName = TEXT("Layer_PORK");
	Setting_ModelNames();
	m_StrObjectFullName = TEXT("Prototpye_GameObject_PORK");
	...
	m_isRender = true;
	//테스트코드
	//m_iCookingNumber = (2 << (_uint)CookType::COOK_BAKE)+ (2 << (_uint)CookType::COOK_CUT);
	return S_OK;
}

_int CPork::Tick(_double TimeDelta)
{
...
_int CPork::LateTick(_double TimeDelta)
{
...
void CPork::Setting_ModelNames()
{
	m_StrModelName[FMT_IDLE] = TEXT("Prototype_Component_Model_Pork0");
	m_StrModelName[FMT_BOILED] = TEXT("Prototype_Component_Model_Pork0");
	m_StrModelName[FMT_CUTTING] = TEXT("Prototype_Component_Model_PorkSliced0");
	m_StrModelName[FMT_BLENDED] = TEXT("Prototype_Component_Model_Pork0");
	m_StrModelName[FMT_GRIDDLED] = TEXT("Prototype_Component_Model_PorkGriddled0");
}

HRESULT CPork::SetUp_Component()
{
	/* Com_Renderer */
...
HRESULT CPork::SetUp_ConstantTable()
{
...
CPork * CPork::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CPork*	pInstance = new CPork(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created : CPork");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CPork::Clone(void * pArg, _uint iCurrentScene)
{
	CPork*	pInstance = new CPork(*this);
