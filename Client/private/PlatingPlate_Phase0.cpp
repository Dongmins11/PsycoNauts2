#include "stdafx.h"
#include "..\public\PlatingPlate_Phase0.h"
#include "GameInstance.h"

CPlatingPlate_Phase0::CPlatingPlate_Phase0(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CGameObject(pDevice,pDeviceContext)
{
}

CPlatingPlate_Phase0::CPlatingPlate_Phase0(const CPlatingPlate_Phase0 & rhs)
	:CGameObject(rhs)
	,m_vecNaviData(rhs.m_vecNaviData)
{
}

HRESULT CPlatingPlate_Phase0::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype()))
		return E_FAIL;

	Loading_PlateNaviData(TEXT("../../Data/Stage03/NaviPlating0.dat"));
	return S_OK;
}

HRESULT CPlatingPlate_Phase0::NativeConstruct(void * pArg, _uint iCurrentScene)
{
	...
	m_StrLayerName = TEXT("Layer_Plating_Plate0");//objinfo.StrLayerName;
	m_StrModelName = TEXT("Prototype_Component_Model_Compt_Plating_Plate0");//objinfo.StrModelName;
	m_StrObjectFullName = TEXT("Prototype_GameObject_Plating_Plate0"); //objinfo.StrObjectFullName;
...
_int CPlatingPlate_Phase0::Tick(_double TimeDelta)
{
...
_int CPlatingPlate_Phase0::LateTick(_double TimeDelta)
{
...
HRESULT CPlatingPlate_Phase0::Render()
{
...
_bool CPlatingPlate_Phase0::Checking_FruitCooking(FruitType eFruitType, CookType eCookType, _uint* iCookingUINumber)
{
	_int iCheckFruit = -1;
...
HRESULT CPlatingPlate_Phase0::SetUp_Component()
{
...
HRESULT CPlatingPlate_Phase0::SetUp_ConstantTable()
{
...
HRESULT CPlatingPlate_Phase0::Loading_PlateNaviData(const _tchar * pNaviDataFilePath)
{
	HANDLE			hFile = 0;
	_uint			iCntPoint = 3;
	_ulong	dwByte = 0;
...
HRESULT CPlatingPlate_Phase0::SetUp_Triggers()
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
	_matrix mat = XMMatrixScaling(5.f, 2.f, 8.f) * XMMatrixIdentity();//XMMatrixTranslation(-4.f, 1.f, 0.f);
	_vector LookVec = mat.r[2];
	LookVec = XMVectorSetW(LookVec, (_float)(2<<(_uint)CookType::COOK_BOIL));
	mat.r[2] = LookVec;
	mat.r[3] = XMVectorSet(232.6f, 2.55f, 192.6f, (_float)FruitType::FRUIT_EGG);
	...
	mat = XMMatrixScaling(5.f,2.f,8.f) * XMMatrixIdentity();//XMMatrixTranslation(-4.f, 1.f, 0.f);
	LookVec = mat.r[2];
	LookVec = XMVectorSetW(LookVec, (_float)(2 << (_uint)CookType::COOK_CUT));
	mat.r[2] = LookVec;
	mat.r[3] = XMVectorSet(240.6f, 2.55f, 192.6f, (_float)FruitType::FRUIT_ONION);
	pGameInstance->Add_ObjectToLayer_GetImmediately(SCENE_STAGE03, TEXT("Prototype_GameObject_FruitTrigger"), SCENE_STAGE03, TEXT("Layer_PlateTrigger"), m_pFruitCollider, &mat);

	m_vecPlateFruits.emplace_back(FruitType::FRUIT_ONION);
	m_vecCookTypes.emplace_back((2 << (_uint)CookType::COOK_CUT));
...
#ifdef _DEBUG
HRESULT CPlatingPlate_Phase0::SetUp_RenderComponentForDebug()
{
...
CPlatingPlate_Phase0 * CPlatingPlate_Phase0::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CPlatingPlate_Phase0*	pInstance = new CPlatingPlate_Phase0(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created : CPlatingPlate_Phase0");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CPlatingPlate_Phase0::Clone(void * pArg, _uint iCurrentScene)
{
	CPlatingPlate_Phase0*	pInstance = new CPlatingPlate_Phase0(*this);
...
void CPlatingPlate_Phase0::Free()
{
	__super::Free();
