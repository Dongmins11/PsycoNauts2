#include "stdafx.h"
#include "..\public\ChargeSingleWave.h"
#include "GameInstance.h"

CChargeSingleWave::CChargeSingleWave(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CGameObject(pDevice, pDeviceContext)
{
}

CChargeSingleWave::CChargeSingleWave(const CChargeSingleWave & rhs)
	: CGameObject(rhs)
{
}

HRESULT CChargeSingleWave::NativeConstruct_Prototype()
{
...
HRESULT CChargeSingleWave::NativeConstruct(void * pArg, _uint iCurrentScene)
{
...
_int CChargeSingleWave::Tick(_double TimeDelta)
{
	if (2.f <= m_fTimeAcc)
	{
		m_isDestroy = true;
...
_int CChargeSingleWave::LateTick(_double TimeDelta)
{
...
HRESULT CChargeSingleWave::Render()
{
...
HRESULT CChargeSingleWave::Call_Inheritance()
{
	...
	LocalMatrix.r[0] = vRight * 0.3f;
	LocalMatrix.r[1] = vUp * 0.3f;
	LocalMatrix.r[2] = vLook * 0.6f;
...
HRESULT CChargeSingleWave::SetUp_Component()
{
	/* Com_Renderer */
...
HRESULT CChargeSingleWave::SetUp_ConstantTable()
{
...
CChargeSingleWave * CChargeSingleWave::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CChargeSingleWave*	pInstance = new CChargeSingleWave(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created : CChargeSingleWave");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CChargeSingleWave::Clone(void * pArg, _uint iCurrentScene)
{
	CChargeSingleWave*	pInstance = new CChargeSingleWave(*this);
