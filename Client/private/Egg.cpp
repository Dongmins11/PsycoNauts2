#include "stdafx.h"
#include "..\public\Egg.h"
#include "Player.h"

CEgg::CEgg(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CFruit(pDevice, pDeviceContext)
{
}

CEgg::CEgg(const CEgg & rhs)
	: CFruit(rhs)
{
}

HRESULT CEgg::NativeConstruct_Prototype()
{
...
HRESULT CEgg::NativeConstruct(void * pArg, _uint iCurrentScene)
{
	...
	m_StrLayerName = TEXT("Layer_EGG");
	Setting_ModelNames();
	m_StrObjectFullName = TEXT("Prototpye_GameObject_EGG");
	...
	//m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(239.f, 4.8f, 190.f, 1.f));
	m_isRender = true;
	...
	//if(pPlayer->Get_Stage03PhaseNum() == 2)
	//	m_iCookingNumber = (2 << (_uint)CookType::COOK_BAKE);
	//RELEASE_INSTANCE(CGameInstance);
	
	return S_OK;
}

_int CEgg::Tick(_double TimeDelta)
{
...
_int CEgg::LateTick(_double TimeDelta)
{
	if (!m_isRender)
		return 0;
...
void CEgg::Setting_ModelNames()
{
	m_StrModelName[FMT_IDLE] = TEXT("Prototype_Component_Model_Egg0");
	m_StrModelName[FMT_BOILED] = TEXT("Prototype_Component_Model_Egg0");
	m_StrModelName[FMT_CUTTING] = TEXT("Prototype_Component_Model_Egg0");
	m_StrModelName[FMT_BLENDED] = TEXT("Prototype_Component_Model_Egg0");
	m_StrModelName[FMT_GRIDDLED] = TEXT("Prototype_Component_Model_EggCracked0");
}

HRESULT CEgg::SetUp_Component()
{
	/* Com_Renderer */
...
CEgg * CEgg::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CEgg*	pInstance = new CEgg(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created : CEgg");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CEgg::Clone(void * pArg, _uint iCurrentScene)
{
